...
public class CustomPhoneKeyboardView extends ViewGroup {
    ...
    private final Runnable onBackButton = () -> {
        if (editCallback != null) {
            editCallback.onRemoveCharacter();
        }
    ...
    private String numberColorKey = Theme.key_windowBackgroundWhiteBlackText;

    public interface EditCallback {
        void onNewCharacter (String symbol);
    ...
    public CustomPhoneKeyboardView(Context context, boolean needDisplayPoint) {
        super(context);

        for (int i = 0; i < 11; i++) {
            if (i == 9 && !needDisplayPoint) {
                continue;
            }
        ...
        backButton.setColorFilter(Theme.getColor(numberColorKey));
        backButton.setBackground(getButtonDrawable(useDarkTheme));
    ...
    public void setUseDarkTheme (boolean useDarkTheme) {
        ...
        this.numberColorKey = useDarkTheme ? Theme.key_wallet_whiteText: Theme.key_windowBackgroundWhiteBlackText;
        updateColors();
    }
    ...
    private static Drawable getButtonDrawable(boolean useDarkTheme) {
        return Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.dp(6),
                Theme.getColor(useDarkTheme ? Theme.key_listSelectorWhite: Theme.key_listSelector),
                ColorUtils.setAlphaComponent(Theme.getColor(useDarkTheme ? Theme.key_listSelectorWhite: Theme.key_listSelector), 60));
    }
    ...
    public void updateColors() {
        ...
        for (View v : views) {
            if (v != null) {
                v.setBackground(getButtonDrawable(useDarkTheme));
    ...
    private final class NumberButtonView extends View {
        ...
        public NumberButtonView(Context context, String number, String symbols) {
            ...
            setBackground(getButtonDrawable(useDarkTheme));
            updateColors();
        }

        private void updateColors() {
            numberTextPaint.setColor(Theme.getColor(numberColorKey));
